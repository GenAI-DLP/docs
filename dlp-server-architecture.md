...
Go 프록시와 dlp-server는 하나의 protobuf 계약(`dlp.proto`)을 공유한다. 이 계약은
**두 컴포넌트의 공유 소스**로 관리하며(예: 공용 서브모듈), dlp-server 측에서 임의로 필드를
추가·변경하지 않는다. 필드 변경은 양 팀 합의와 프록시 재조율을 전제로 한다.

```protobuf
service DLPInspector {
  rpc Inspect (InspectRequest) returns (Verdict);   // unary. 스트리밍 아님
}
...
시작한다. 부록 A의 PostgreSQL 스키마는 네 개 데이터 계약의 단일 기준이며, 프로덕션 및 여유 시점의
목표다.
...
데모는 인메모리/SQLite로 시작하되, 이 DDL이 네 개 데이터 계약(session-context / token-vault /
policy / log-event)의 단일 기준이다. 운영·볼트·감사 3계층의 외래키를 분리해 세션 TTL 삭제가 감사
로그를 지우는 문제를 제거했다.

```sql
-- =====================================================================
-- dlp-server PostgreSQL 스키마
-- 데이터 3계층:
...
- `token_vault`와 `token_vault_access_log`는 FK 없이 `token_id`로만 연결된다. 볼트 하드 삭제 후에도
  접근 이력은 남지만 조인 시 dangling이 된다(감사 우선의 의도된 결과). 필요하면 access log에
  엔티티 타입 등 스냅샷 컬럼을 더해 조인 없이 읽히게 한다.
- `entity_type_ref` / `purpose_ref`는 seed로 시작한다. 목록 확정 시 행만 추가하며, 코드와 DB 중
  하나를 단일 기준으로 정한다.
- `condition_expr` 파서는 `injection.hit`와 `risk_score >= N` 두 종류만 지원하는 소형 매칭으로
  충분하다. `eval()`을 쓰지 않는다.
- `action_type`의 `generalize` / `synthetic` 네이밍을 변환 계층 코드와 문서에서 동일하게 유지한다.
```
